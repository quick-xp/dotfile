...
| `Ctrl+w 4/5/6/7` | ペインリサイズ 50px（上/下/左/右） |
| `Ctrl+w 8/9` | ペインリサイズ 200px（左/右） |
| `Ctrl+w x` | ペインを閉じる |
| `Ctrl+w z` / `Ctrl+w 1` | ペインズーム |
| `Ctrl+w =` | ペインサイズ均等化 |
| `Ctrl+w o` / `Ctrl+w O` | 次/前のペイン（巡回） |
...
|------|------|
| `Ctrl+w c` | 新規タブ |
| `Ctrl+w n` / `Ctrl+w p` | 次/前のタブ |
| `Ctrl+w qq` | タブを閉じる（2回押し） |
| `Ctrl+w ,` | タブ名変更 |
| `Ctrl+w >` / `Ctrl+w <` | タブを右/左に移動 |
| `Ctrl+e 1-8` | タブに直接移動 |
| `Ctrl+e ]` / `Ctrl+e [` | タブを右/左に1つ移動 |
| `Ctrl+e =` / `Ctrl+e -` | タブを右/左に2つ移動 |
| `Ctrl+e '` / `Ctrl+e ;` | タブを右/左に3つ移動 |
| `Cmd+1-9` | タブに直接移動（macOS） |
...
| `Shift+Home/End` | 最上部/最下部 |

### その他

| キー | 説明 |
|------|------|
| `Ctrl+p` | ウィンドウ表示/非表示（グローバル） |
| `Ctrl++` / `Ctrl+-` | 文字サイズ拡大/縮小 |
| `Ctrl+0` | 文字サイズリセット |
| `Ctrl+w r` | 設定リロード |
| `Ctrl+u` | コマンドラインクリア |
